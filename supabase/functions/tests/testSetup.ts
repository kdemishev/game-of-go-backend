...
export async function setupGameWithCaptureGroupInTheField(procedure: string) : Promise<string>{
    ...
    const { data: prepareData, error: prepareError } = await supabase.rpc("execute_prepared_procedure", 
        {
          procedure_input: procedure, 
          game_uuid_input: gameData.uuid 
        });
